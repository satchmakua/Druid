...
class Verderer:
    ...
    def _latest_observation_for(self, target_id: str) -> Observation | None:
        ...
        for entry in self.log.entries():
            record = entry.record
            if record.get("schema") == "verderer.observation/v1" and record.get("target_id") == target_id:
                latest = record
        ...
        try:
            ...
            if is_first:
                ...
                raise RuntimeError(
                    ...
                    f"exists (stale validator); clear verderer-data/politeness-state.json and re-observe"
                ) from None
    ...
    def gossip_bundle(self, old_checkpoint: str) -> dict:
        """A self-contained `verderer.consistency/v1` proving the current checkpoint extends the
        client's earlier `old_checkpoint` (M13 gossip). It carries both signed checkpoints and
        a C2SP consistency proof, so `verderer-verify consistency` confirms — offline — that the
        ...
        return {
            "schema": "verderer.consistency/v1",
            "origin": "verderer.watchdog/m1-log",
            "from": old_size,
            "to": new_size,
    ...
    def bundle(self, target_id: str, index: int | None = None) -> dict:
        """Assemble a self-verifying `verderer.proofbundle/v1` for an observation (DESIGN §6.4).
        ...
        verifies offline via `verderer-verify bundle` — trusting neither the source nor Verderer.
        """
        observations = [
            entry
            for entry in self.log.entries()
            if entry.record.get("schema") == "verderer.observation/v1"
