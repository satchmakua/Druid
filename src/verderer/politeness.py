...
robots-aware, rate-limited, backed-off, and conditional — so Verderer never becomes a load
...
class ConditionalFetcher(Protocol):
    """The network seam the polite fetcher wraps: like :class:`~verderer.collectors.base.Fetcher`
...
class RenderFetcher(Protocol):
    """A headless-render seam (:class:`~verderer.collectors.base.RenderEngine`) the polite
...
class PolitenessPolicy:
    ...
    def _load_validators(self) -> dict[str, dict[str, str]]:
        ...
        try:
            ...
            # (`verderer verify` / `log`), which do not depend on politeness at all. Discard
    ...
    def fetcher(self, inner: ConditionalFetcher) -> Fetcher:
        """Adapt this policy into a :class:`~verderer.collectors.base.Fetcher` the static
    ...
    def engine(self, inner: RenderFetcher) -> RenderEngine:
        """Adapt this policy into a :class:`~verderer.collectors.base.RenderEngine`."""
