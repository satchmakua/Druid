...
from druid.hashing import multihash_sha256
from druid.pipeline import Druid
from druid.warc import archived_payload, build_warc, iter_records
...
def _druid(tmp_path: Path, fetcher: object) -> Druid:
    return Druid(
        tmp_path / "data",
        targets={"t": TARGET},
...
def test_dedup_does_not_archive_a_second_identical_warc(tmp_path: Path, ledger_built: None) -> None:
    ...
    second = druid.observe("t")  # byte-identical -> deduped before a WARC is built
    assert second.status == "unchanged" and second.observation is None
    assert druid.store.has(warc_hash)  # the one WARC from the baseline is still there
...
def test_export_ships_the_warcs(tmp_path: Path, ledger_built: None) -> None:
    ...
    info = export_site(druid, tmp_path / "site")
    assert info["warcs"] == 1
...
def test_record_omits_warc_link_when_blob_is_missing(tmp_path: Path, ledger_built: None) -> None:
    ...
    druid.store._path(obs.warc_record_hash).unlink()

    view = build_record(druid)["targets"][0]["observations"][0]
...
def test_render_observation_is_archived_as_resource(tmp_path: Path, ledger_built: None) -> None:
    from druid.collectors.base import RenderResult
    from druid.collectors.render import RenderCollector
    ...
    obs = druid.observe("tool").observation
    assert obs is not None and obs.warc_record_hash is not None
    warc = druid.store.get(obs.warc_record_hash)
