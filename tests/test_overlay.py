...
and Annals is badged annals-attested with a downloadable proof bundle; a third-party-only
...
from annals.overlay import OverlayCapture, WaybackSource, build_overlay, write_overlay
from annals.pipeline import Annals

TARGET_URL = "https://www.example.gov/ghg"
SIBLING_URL = "https://example.gov/other-page"  # archived by a third party, never observed by Annals
...
def _annals(tmp_path: Path) -> Annals:
...
def test_attested_resource_is_badged_and_third_party_only_is_not(tmp_path: Path, ledger_built: None) -> None:
    annals = _annals(tmp_path)
    annals.observe("t")  # Annals now attests TARGET_URL

    # The archive returns a capture of the observed URL AND of a sibling Annals never saw.
    source = FakeSource(
        {TARGET_URL: [_cap("http://example.gov/ghg", "20250101000000"), _cap(SIBLING_URL, "20250102000000")]}
    )
    overlay = build_overlay(annals, [source], query_urls=[TARGET_URL])
    ...
    assert attested["badge"] == "annals-attested"
    # The bundle is keyed on the specific attested leaf, so it proves exactly this hash.
    assert attested["annals"]["bundle"] == f"bundles/{attested['annals']['index']}.json"
    assert attested["annals"]["observations"] == 1
    ...
    assert sibling["attested"] is False and sibling["badge"] is None and sibling["annals"] is None
    assert sibling["external"][0]["captures"][0]["archive_url"].startswith("http://web.archive.org/web/")

    assert overlay["attested_count"] == 1
    assert overlay["schema"] == "annals.overlay/v1"
...
def test_write_overlay_emits_index_and_downloadable_bundle(tmp_path: Path, ledger_built: None) -> None:
    ...
    out = tmp_path / "site"
    info = write_overlay(annals, out, [source], query_urls=[TARGET_URL])
    ...
    bundle = json.loads((out / att["annals"]["bundle"]).read_text(encoding="utf-8"))
    assert bundle["schema"] == "annals.proofbundle/v1"  # a real, self-verifying proof bundle
    assert bundle["observation"]["url"] == TARGET_URL
    # The invariant: the shipped bundle proves exactly the hash the row advertises.
    assert bundle["observation"]["raw_bytes_hash"] == att["annals"]["content_hash"]


def test_attested_only_resource_appears_without_any_archive(tmp_path: Path, ledger_built: None) -> None:
    # A Annals-attested resource that no third party archived is still in the overlay,
    # badged, with an empty external list.
    annals = _annals(tmp_path)
    annals.observe("t")
    overlay = build_overlay(annals, [FakeSource({})], query_urls=[TARGET_URL])
...
def test_every_attested_bundle_proves_its_advertised_hash(tmp_path: Path, ledger_built: None) -> None:
    ...
    data = tmp_path / "data"
    Annals(data, targets={"t": Target(id="t", title="T", url=url1)}, terms=[],
          collector=StaticCollector(fetcher=fetch_for(url1))).observe("t")
    annals = Annals(data, targets={"t": Target(id="t", title="T", url=url2)}, terms=[],
                  collector=StaticCollector(fetcher=fetch_for(url2)))
    annals.observe("t")  # same ledger now holds observations of url1 (v1) and url2 (v2)
    ...
    out = tmp_path / "site"
    write_overlay(annals, out, [source], query_urls=[url1, url2])
    overlay = json.loads((out / "overlay.json").read_text(encoding="utf-8"))

    attested = [r for r in overlay["resources"] if r["attested"]]
    assert len(attested) == 2  # two distinct URL resources, both attested
    for row in attested:
        bundle = json.loads((out / row["annals"]["bundle"]).read_text(encoding="utf-8"))
        # No overclaim: the downloadable proof attests exactly this resource's URL + hash —
        # and the URL shown is the one Annals observed, not a third party's capture form.
        assert bundle["observation"]["url"] == row["url"]
        assert bundle["observation"]["raw_bytes_hash"] == row["annals"]["content_hash"]
