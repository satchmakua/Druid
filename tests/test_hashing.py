from druid.hashing import multihash_sha256, verify_multihash
...
def test_verify_roundtrip() -> None:
    mh = multihash_sha256(b"druid")
    assert verify_multihash(b"druid", mh)
    assert not verify_multihash(b"druid!", mh)
