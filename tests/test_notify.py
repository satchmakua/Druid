...
def test_webhook_notifier_posts_alert_payload() -> None:
    ...
    assert payload["schema"] == "druid.alert/v1"
...
def test_email_message_is_built_and_sent_via_injected_sender() -> None:
    captured: list = []
    notifier = SmtpEmailNotifier(sender=captured.append, from_addr="druid@watch")
...
def test_load_subscriptions_from_repo_config() -> None:
    from druid.config import load_targets  # noqa: F401  (repo layout sanity)

    path = Path(__file__).resolve().parents[1] / "data" / "subscriptions.toml"
    from druid.notify import load_subscriptions
