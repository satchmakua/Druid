...
from verderer.anchors import OpensslTsaAnchorer
...
def _verify_bundle(bundle: dict, tmp_path: Path, root: Path, name: str) -> subprocess.CompletedProcess[str]:
    ...
    return subprocess.run(
        [str(find_binary("verderer-verify")), "bundle", str(path), "--root", str(root)],
        capture_output=True,
        encoding="utf-8",
    )
...
def test_unpinned_anchor_is_reported_not_fatal(tmp_path: Path, ledger_built: None, openssl_available: None) -> None:
    ...
    verderer = _make_verderer(tmp_path)
    verderer.observe("t")
    verderer.anchor(OpensslTsaAnchorer(tmp_path / "data" / "anchors" / "dev-tsa"))
    bundle = verderer.bundle("t")
...
def test_live_digicert_anchor_verifies_with_embedded_root(
    ...
    from verderer.anchors import HttpTsaAnchorer

    verderer = _make_verderer(tmp_path)
    verderer.observe("t")
    try:
        verderer.anchor(HttpTsaAnchorer("digicert"))
